  ...
  */
  ...
  static class LinearGradientPaint implements Paint {
    ...
    public LinearGradientPaint(double x1, double y1, double x2, double y2,
                               double[] offset, int[] color, int count,
                               double opacity) {
      this.x1 = x1;
      this.y1 = y1;
      this.x2 = x2;
    ...
    public PaintContext createContext(ColorModel cm,
      ...
      return new LinearGradientContext((double) t1.getX(), (double) t1.getY(),
        ...
        if (span <= 0) {
          ...
          for (int i = 1; i < count; i++) {
            ...
            for (int j = prev; j <= last; j++) {
              double btwn = PApplet.norm(j, prev, last);
  ...
  static class RadialGradientPaint implements Paint {
    ...
    public RadialGradientPaint(double cx, double cy, double radius,
                               double[] offset, int[] color, int count,
                               double opacity) {
      this.cx = cx;
      this.cy = cy;
    ...
    public class RadialGradientContext implements PaintContext {
      ...
      public Raster getRaster(int x, int y, int w, int h) {
        ...
        for (int j = 0; j < h; j++) {
          for (int i = 0; i < w; i++) {
            double distance = PApplet.dist(cx, cy, x + i, y + j);
            int which = PApplet.min((int) (distance * ACCURACY), interp.length-1);
