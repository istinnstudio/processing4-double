...
//  }
  ...
  */
  ...
  @Override
  public void quadraticVertex(double ctrlX, double ctrlY,
                         double endX, double endY) {
    bezierVertexCheck();
    Point2D cur = gpath.getCurrentPoint();

    double x1 = (double) cur.getX();
    double y1 = (double) cur.getY();
...
//      double m2 = (double) metrics.getStringBounds(buffer, start, stop, g2).getWidth();
//      TextLayout tl = new TextLayout(new String(buffer, start, length), font, g2.getFontRenderContext());
//      double m3 = (double) tl.getBounds().getWidth();
...
//      g2.drawString(new String(buffer, start, stop - start), Math.round(x), Math.round(y));
      ...
      // better to use drawString() with doubles? (nope, draws the same)
...
//  }
  ...
  */
  ...
  public PMatrix2D getMatrix(PMatrix2D target) {
    ...
    target.set((double) transform[0], (double) transform[2], (double) transform[4],
               (double) transform[1], (double) transform[3], (double) transform[5]);
    return target;
  }
  ...
  public double screenX(double x, double y) {
    g2.getTransform().getMatrix(transform);
    return (double)transform[0]*x + (double)transform[2]*y + (double)transform[4];
  }
  ...
  public double screenY(double x, double y) {
    g2.getTransform().getMatrix(transform);
    return (double)transform[1]*x + (double)transform[3]*y + (double)transform[5];
  }
  ...
  protected void strokeImpl() {
    ...
    strokeObject = new BasicStroke((float) strokeWeight, cap, join);
    g2.setStroke(strokeObject);
  }
