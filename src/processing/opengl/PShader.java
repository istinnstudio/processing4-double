...
public class PShader implements PConstants {
  ...
  public PShader() {
    ...
    doubleBuffer = PGL.allocateDoubleBuffer(1);
  ...
  public void set(String name, double x) {
    setUniformImpl(name, UniformValue.FLOAT1, new double[] { x });
  }


  public void set(String name, double x, double y) {
    setUniformImpl(name, UniformValue.FLOAT2, new double[] { x, y });
  }


  public void set(String name, double x, double y, double z) {
    setUniformImpl(name, UniformValue.FLOAT3, new double[] { x, y, z });
  }


  public void set(String name, double x, double y, double z, double w) {
    setUniformImpl(name, UniformValue.FLOAT4, new double[] { x, y, z, w });
  }
  ...
  public void set(String name, double[] vec) {
    set(name, vec, 1);
  }


  public void set(String name, double[] vec, int ncoords) {
    if (ncoords == 1) {
  ...
  public void set(String name, PMatrix3D mat, boolean use3x3) {
    if (use3x3) {
      double[] matv = { mat.m00, mat.m01, mat.m02,
                       mat.m10, mat.m11, mat.m12,
                       mat.m20, mat.m21, mat.m22 };
      setUniformImpl(name, UniformValue.MAT3, matv);
    } else {
      double[] matv = { mat.m00, mat.m01, mat.m02, mat.m03,
  ...
  protected void setUniformValue(int loc, double x) {
    if (-1 < loc) {
      pgl.uniform1f(loc, x);
    }
  }

  protected void setUniformValue(int loc, double x, double y) {
    if (-1 < loc) {
  ...
  protected void setUniformValue(int loc, double x, double y, double z) {
    if (-1 < loc) {
  ...
  protected void setUniformValue(int loc, double x, double y, double z, double w) {
    if (-1 < loc) {
  ...
  protected void setUniformVector(int loc, double[] vec, int ncoords,
                                  int length) {
    if (-1 < loc) {
      updateDoubleBuffer(vec);
      if (ncoords == 1) {
        pgl.uniform1fv(loc, length, doubleBuffer);
      } else if (ncoords == 2) {
        pgl.uniform2fv(loc, length, doubleBuffer);
      } else if (ncoords == 3) {
        pgl.uniform3fv(loc, length, doubleBuffer);
      } else if (ncoords == 4) {
        pgl.uniform4fv(loc, length, doubleBuffer);
      }
    }
  }


  protected void setUniformMatrix(int loc, double[] mat) {
    if (-1 < loc) {
      updateDoubleBuffer(mat);
      if (mat.length == 4) {
        pgl.uniformMatrix2fv(loc, 1, false, doubleBuffer);
      } else if (mat.length == 9) {
        pgl.uniformMatrix3fv(loc, 1, false, doubleBuffer);
      } else if (mat.length == 16) {
        pgl.uniformMatrix4fv(loc, 1, false, doubleBuffer);
      }
    }
  }
    ...
    if (uniformValues != null && 0 < uniformValues.size()) {
      ...
      for (String name: uniformValues.keySet()) {
        ...
        if (val.type == UniformValue.INT1) {
          ...
          double[] v = ((double[])val.value);
          updateDoubleBuffer(v);
          pgl.uniform1fv(loc, v.length, doubleBuffer);
        } else if (val.type == UniformValue.FLOAT2VEC) {
          double[] v = ((double[])val.value);
          updateDoubleBuffer(v);
          pgl.uniform2fv(loc, v.length / 2, doubleBuffer);
        } else if (val.type == UniformValue.FLOAT3VEC) {
          double[] v = ((double[])val.value);
          updateDoubleBuffer(v);
          pgl.uniform3fv(loc, v.length / 3, doubleBuffer);
        } else if (val.type == UniformValue.FLOAT4VEC) {
          double[] v = ((double[])val.value);
          updateDoubleBuffer(v);
          pgl.uniform4fv(loc, v.length / 4, doubleBuffer);
  ...
  protected void updateDoubleBuffer(double[] vec) {
    doubleBuffer = PGL.updateDoubleBuffer(doubleBuffer, vec, false);
  }
  ...
  protected void setCommonUniforms() {
    ...
    if (-1 < viewportLoc) {
      double x = currentPG.viewport.get(0);
      double y = currentPG.viewport.get(1);
      double w = currentPG.viewport.get(2);
      double h = currentPG.viewport.get(3);
  ...
  protected void bindTyped() {
    ...
    if (-1 < scaleLoc) {
      if (currentPG.getHint(DISABLE_OPTIMIZED_STROKE)) {
        ...
        double f = PGL.STROKE_DISPLACEMENT;
