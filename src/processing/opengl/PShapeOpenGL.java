...
public class PShapeOpenGL extends PShape {
  ...
  protected double curveTightness;

  protected int savedBezierDetail;
  protected int savedCurveDetail;
  protected double savedCurveTightness;

  // ........................................................

  // Normals

  protected double normalX, normalY, normalZ;
  ...
  protected double savedStrokeWeight;
  ...
  protected double savedShininess;
  ...
  public PShapeOpenGL(PGraphicsOpenGL pg, int family) {
    ...
    strokeWeight = (float) pg.strokeWeight;
  ...
  public PShapeOpenGL(PGraphicsOpenGL pg, int kind, double... p) {
    this(pg, PRIMITIVE);
    setKind(kind);
    setParams(p);
  }
...
*/
  ...
  public double getWidth() {
    PVector min = new PVector(Double.POSITIVE_INFINITY, Double.POSITIVE_INFINITY,
                              Double.POSITIVE_INFINITY);
    PVector max = new PVector(Double.NEGATIVE_INFINITY, Double.NEGATIVE_INFINITY,
  ...
  protected void setTextureModeImpl(int mode) {
    ...
    textureMode = mode;
    if (image != null) {
      double uFactor = image.width;
      double vFactor = image.height;
  ...
  protected void scaleTextureUV(double uFactor, double vFactor) {
    if (PGraphicsOpenGL.same(uFactor, 1) &&
        PGraphicsOpenGL.same(vFactor, 1)) return;

    for (int i = 0; i < inGeo.vertexCount; i++) {
      double u = inGeo.texcoords[2 * i + 0];
      double v = inGeo.texcoords[2 * i + 1];
    ...
    if (shapeCreated && tessellated && hasPolys) {
      ...
      for (int i = firstLineVertex; i < last1; i++) {
        double u = tessGeo.polyTexCoords[2 * i + 0];
        double v = tessGeo.polyTexCoords[2 * i + 1];
  ...
  @Override
  public void setParams(double[] source) {
    if (family != PRIMITIVE) {
  ...
  @Override
  public void setPath(int vcount, double[][] verts, int ccount, int[] codes) {
    if (family != PATH) {
  ...
  protected void transform(int type, double... args) {
...
//    pg.curveTightness(tightness);
  ...
  protected void setStrokeWeightImpl(float weight) {
    ...
    double oldWeight = strokeWeight;
    strokeWeight = weight;

    Arrays.fill(inGeo.strokeWeights, 0, inGeo.vertexCount, strokeWeight);
    if (shapeCreated && tessellated && (hasLines || hasPoints)) {
      double resizeFactor = weight / oldWeight;
      if (hasLines) {
        if (is3D()) {
  ...
  protected void setShininessImpl(double shininess) {
  ...
  public double getAttribPositionX(String name, int index) {
    VertexAttribute attrib = polyAttribs.get(name);
    if (attrib == null)
      throw new RuntimeException("Trying to get values of non existing attribute");
  ...
  public double getAttribPositionY(String name, int index) {
    VertexAttribute attrib = polyAttribs.get(name);
    if (attrib == null)
      throw new RuntimeException("Trying to get values of non existing attribute");
  ...
  public double getAttribPositionZ(String name, int index) {
    VertexAttribute attrib = polyAttribs.get(name);
    if (attrib == null)
      throw new RuntimeException("Trying to get values of non existing attribute");
  ...
  public double getAttribNormalX(String name, int index) {
    VertexAttribute attrib = polyAttribs.get(name);
    if (attrib == null)
      throw new RuntimeException("Trying to get values of non existing attribute");
  ...
  public double getAttribNormalY(String name, int index) {
    VertexAttribute attrib = polyAttribs.get(name);
    if (attrib == null)
      throw new RuntimeException("Trying to get values of non existing attribute");
  ...
  public double getAttribNormalZ(String name, int index) {
    VertexAttribute attrib = polyAttribs.get(name);
    if (attrib == null)
      throw new RuntimeException("Trying to get values of non existing attribute");
  ...
  public double[] getAttrib(String name, int index, double[] values) {
    VertexAttribute attrib = polyAttribs.get(name);
    if (attrib == null)
      throw new RuntimeException("Trying to get values of non existing attribute");

    if (root.tessUpdate) {
      if (values == null || values.length < attrib.tessSize) values = new double[attrib.tessSize];
      double[] tessAttrib = tessGeo.fpolyAttribs.get(name);
      int tessIdx = firstPolyVertex + index;
      PApplet.arrayCopy(tessAttrib, attrib.tessSize * tessIdx, values, 0, attrib.tessSize);
    } else {
      if (values == null || values.length < attrib.size) values = new double[attrib.size];
      double[] array = inGeo.fattribs.get(name);
  ...
  public void setAttribPosition(String name, int index, double x, double y, double z) {
    if (openShape) {
  ...
  public void setAttribNormal(String name, int index, double nx, double ny, double nz) {
    if (openShape) {
    ...
    if (0 < tessGeo.polyVertexCount) {
      ...
      for (int n = firstPolyIndexCache; n <= lastPolyIndexCache; n++) {
        ...
        for (int tr = ioffset / 3; tr < (ioffset + icount) / 3; tr++) {
          ...
          if (is3D()) {
            double x0 = vertices[4 * i0 + 0];
            double y0 = vertices[4 * i0 + 1];
            double z0 = vertices[4 * i0 + 2];
            double x1 = vertices[4 * i1 + 0];
            double y1 = vertices[4 * i1 + 1];
            double z1 = vertices[4 * i1 + 2];
            double x2 = vertices[4 * i2 + 0];
            double y2 = vertices[4 * i2 + 1];
            double z2 = vertices[4 * i2 + 2];

            double nx0 = attribs[3 * i0 + 0];
            double ny0 = attribs[3 * i0 + 1];
            double nz0 = attribs[3 * i0 + 2];
            double nx1 = attribs[3 * i1 + 0];
            double ny1 = attribs[3 * i1 + 1];
            double nz1 = attribs[3 * i1 + 2];
            double nx2 = attribs[3 * i2 + 0];
            double ny2 = attribs[3 * i2 + 1];
            double nz2 = attribs[3 * i2 + 2];
            ...
            double x0 = vertices[4 * i0 + 0], y0 = vertices[4 * i0 + 1];
            double x1 = vertices[4 * i1 + 0], y1 = vertices[4 * i1 + 1];
            double x2 = vertices[4 * i2 + 0], y2 = vertices[4 * i2 + 1];
    ...
    if (0 < tessGeo.lineVertexCount) {
      ...
      for (int n = firstLineIndexCache; n <= lastLineIndexCache; n++) {
        ...
        for (int ln = ioffset / 6; ln < (ioffset + icount) / 6; ln++) {
          ...
          lineTess.strokeWeight((float) sw0);
          lineTess.stroke(argb0);
          lineTess.vertex(pt0[X], pt0[Y], pt0[Z]);
          lineTess.strokeWeight((float) sw1);
    ...
    if (0 < tessGeo.pointVertexCount) {
      ...
      for (int n = 0; n < cache.size; n++) {
        ...
        while (pt < (ioffset + icount) / 3) {
          ...
          pointTess.strokeWeight((float) weight);
  ...
  public void enableStyle() {
    if (savedStroke) {
      ...
      setStrokeWeight((float) savedStrokeWeight);
  ...
  protected void styles(PGraphics g) {
    if (g instanceof PGraphicsOpenGL) {
      if (g.stroke) {
        ...
        setStrokeWeight((float) g.strokeWeight);
  ...
  private void inGeoToVertices() {
    ...
    if (inGeo.codeCount == 0) {
      for (int i = 0; i < inGeo.vertexCount; i++) {
        int index = 3 * i;
        double x = inGeo.vertices[index++];
        double y = inGeo.vertices[index  ];
        super.vertex(x, y);
      }
    } else {
      int v;
      ...
      for (int tr = ioffset / 3; tr < (ioffset + icount) / 3; tr++) {
        ...
        if (textureImage != null) {
          ...
          if (raw.is3D()) {
            ...
            double sx0 = g.screenXImpl(pt0[0], pt0[1], pt0[2], pt0[3]);
            double sy0 = g.screenYImpl(pt0[0], pt0[1], pt0[2], pt0[3]);
            double sx1 = g.screenXImpl(pt1[0], pt1[1], pt1[2], pt1[3]);
            double sy1 = g.screenYImpl(pt1[0], pt1[1], pt1[2], pt1[3]);
            double sx2 = g.screenXImpl(pt2[0], pt2[1], pt2[2], pt2[3]);
            double sy2 = g.screenYImpl(pt2[0], pt2[1], pt2[2], pt2[3]);
            raw.fill(argb0);
