...
public class LinePath {
  ...
  private static double defaultMiterlimit = 10.0f;
   ...
   * @see LinePath#moveTo
   */
  public final void moveTo(double x, double y, int c) {
   ...
   * coordinates to the new specified coordinates specified in double precision.
   * <p>
   * This method provides a single precision variant of the double precision
   ...
   * @see LinePath#lineTo
   */
  public final void lineTo(double x, double y, int c) {
    needRoom(true, 1);
  ...
  static public class PathIterator {
    ...
    PathIterator(LinePath p2df) {
      this.path = p2df;
      this.doubleCoords = p2df.doubleCoords;
      pointIdx = 0;
      colorIdx = 0;
    }
  ...
  static public LinePath createStrokedPath(LinePath src, double weight,
                                           int caps, int join) {
  ...
  static public LinePath createStrokedPath(LinePath src, double weight,
                                           int caps, int join, double miterlimit) {
  ...
  static public LinePath createStrokedPath(LinePath src, double weight,
                                           int caps, int join,
                                           double miterlimit, PMatrix2D transform) {
    ...
    strokeTo(src, weight, caps, join, miterlimit, transform, new LineStroker() {
      ...
      public void moveTo(int x0, int y0, int c0) {
        dest.moveTo(S15_16ToDouble(x0), S15_16ToDouble(y0), c0);
      }
      ...
      public void lineTo(int x1, int y1, int c1) {
        dest.lineTo(S15_16ToDouble(x1), S15_16ToDouble(y1), c1);
      }
  ...
  private static void strokeTo(LinePath src, double width, int caps, int join,
                               double miterlimit, PMatrix2D transform,
                               LineStroker lsink) {
    lsink = new LineStroker(lsink, DoubleToS15_16(width), caps, join,
                            DoubleToS15_16(miterlimit),
    ...
    while (!pi.isDone()) {
      ...
      case SEG_MOVETO:
        color = ((int)coords[2]<<24) |
                ...
                ((int)coords[4]<< 8) |
                 (int)coords[5];
        lsink.moveTo(DoubleToS15_16(coords[0]), DoubleToS15_16(coords[1]), color);
        break;
      case SEG_LINETO:
        color = ((int)coords[2]<<24) |
                ((int)coords[3]<<16) |
                ((int)coords[4]<< 8) |
                 (int)coords[5];
        lsink.lineJoin();
        lsink.lineTo(DoubleToS15_16(coords[0]), DoubleToS15_16(coords[1]), color);
        break;
      case SEG_CLOSE:
        lsink.lineJoin();
        lsink.close();
  ...
  static int DoubleToS15_16(double flt) {
    flt = flt * 65536f + 0.5f;
  ...
  static double S15_16ToDouble(int fix) {
    return (fix / 65536f);
  }
}
