...
public class Table {
  ...
  protected double missingFloat = Float.NaN;
    ...
    try {
      ...
      while (rs.next()) {
        for (int col = 0; col < columnCount; col++) {
          ...
          case FLOAT: setDouble(row, col, rs.getDouble(col+1)); break;
...
//        <table:table-cell table:formula="of:=SUM([.E7:.E8])" office:value-type="double" office:value="4150">
...
//        writer.print("&apos;");
  ...
  void saveNumberODS(OutputStream output, String text) throws IOException {
    writeUTF(output,
             "          <table:table-cell office:value-type=\"double\" office:value=\"" + text + "\">",
             "            <text:p>" + text + "</text:p>",
             "          </table:table-cell>");
  }
...
//      PApplet.splice(columnCategories, new HashMapBlows(), index);
    ...
    switch (type) {
      ...
      case FLOAT: columns[index] = new double[rowCount]; break;
 ...
 /**
   ...
   * @param columnType One of int, long, double, double, string, or category.
   */
   ...
   * @param newType something fresh, maybe try an int or a double for size?
   */
  public void setColumnType(int column, int newType) {
    switch (newType) {
      ...
      case FLOAT: {
        ...
        for (int row = 0; row < rowCount; row++) {
          String s = getString(row, column);
          doubleData[row] = (s == null) ? missingDouble : PApplet.parseDouble(s, missingDouble);
        }
...
//          setColumnType(i, dictionary.getString(i, typeCol));
    ...
    if (newCount != rowCount) {
      ...
      for (int col = 0; col < columns.length; col++) {
        switch (columnTypes[col]) {
          ...
          case FLOAT: columns[col] = PApplet.expand((double[]) columns[col], newCount); break;
   ...
   * <b>setDouble()</b>, or <b>setString()</b>. If a <b>TableRow</b> object is
...
//          System.arraycopy(doubleData, dead+1, doubleTemp, dead, (rowCount - dead) + 1);
  ...
  protected void setRowCol(int row, int col, String piece) {
    ...
    case FLOAT:
      double[] doubleData = (double[]) columns[col];
      doubleData[row] = PApplet.parseDouble(piece, missingDouble);
      break;
    case DOUBLE:
      double[] doubleData = (double[]) columns[col];
      try {
...
//          stringData[row] = (String) piece;
      ...
      case FLOAT:
        ...
        if (piece == null) {
          floatData[row] = (float) missingDouble;
        } else if (piece instanceof Double) {
          floatData[row] = (Float) piece;
        } else {
          floatData[row] = (float) PApplet.parseDouble(String.valueOf(piece), missingDouble);
        }
        break;
      case DOUBLE:
        double[] doubleData = (double[]) columns[col];
  ...
  static class RowPointer implements TableRow {
    ...
    public void setFloat(String columnName, double value) {
    ...
    public void write(PrintWriter writer) {
      ...
      @Override
      public void setFloat(int column, double value) {
         throw new UnsupportedOperationException("Not supported yet."); // Generated from nbfs://nbhost/SystemFileSystem/Templates/Classes/Code/GeneratedMethodBody
      }
  ...
  */
  ...
  /**
   * Retrieves a double value from the <b>Table</b>'s specified row and column.
   * The row is specified by its ID, while the column may be specified by either
   * its ID or title.
   *
   * @webref table:method
   * @webBrief Retrieves a double value from the <b>Table</b>'s specified row and column
   * @param row ID number of the row to reference
   * @param column ID number of the column to reference
   * @see Table#getInt(int, int)
   * @see Table#getString(int, int)
  ...
  public double getDouble(int row, int column) {
    ...
    return PApplet.parseDouble(str, missingDouble);
  }
  ...
  /**
   * Stores a double value in the <b>Table</b>'s specified row and column.
   * The row is specified by its ID, while the column may be specified by
   * either its ID or title.
   *
   * @webref table:method
   * @webBrief Stores a double value in the <b>Table</b>'s specified row and column
   * @param row ID number of the target row
   * @param column ID number of the target column
   * @param value value to assign
   * @see Table#setInt(int, int, int)
  ...
  public void setDouble(int row, int column, double value) {
    if (columnTypes[column] == STRING) {
      ...
      double[] longData = (double[]) columns[column];
      longData[row] = value;
    }
  }
  ...
  public void setMissingFloat(double value) {
    missingFloat = value;
  }
  ...
  public void setFloat(int row, String columnName, double value) {
    setFloat(row, getColumnIndex(columnName), (float) value);
  }


  public double[] getFloatColumn(String name) {
    int col = getColumnIndex(name);
    return (col == -1) ? null : getFloatColumn(col);
  }


  public double[] getFloatColumn(int col) {
    double[] outgoing = new double[rowCount];
    for (int row = 0; row < rowCount; row++) {
  ...
  public double[] getFloatRow(int row) {
    double[] outgoing = new double[columns.length];
    for (int col = 0; col < columns.length; col++) {
...
//      setDouble(row, column, ((Double) value).doubleValue());
...
//  }
  ...
  protected void sort(final int column, final boolean reverse) {
    ...
    Sort s = new Sort() {
      ...
      public int compare(int index1, int index2) {
        ...
        case FLOAT:
          double difff = getDouble(a, column) - getDouble(b, column);
    ...
    for (int col = 0; col < getColumnCount(); col++) {
      ...
      case LONG:
        ...
        break;
      case FLOAT:
        double[] oldFloat = (double[]) columns[col];
        double[] newFloat = new double[rowCount];
        for (int row = 0; row < getRowCount(); row++) {
  ...
  public DoubleList getDoubleList(String columnName) {
    return new DoubleList(getDoubleColumn(columnName));
  }


  public DoubleList getDoubleList(int column) {
    return new DoubleList(getDoubleColumn(column));
  }
  ...
  public DoubleDict getDoubleDict(String keyColumnName, String valueColumnName) {
    return new DoubleDict(getStringColumn(keyColumnName),
                         getDoubleColumn(valueColumnName));
  }


  public DoubleDict getDoubleDict(int keyColumn, int valueColumn) {
    return new DoubleDict(getStringColumn(keyColumn),
                         getDoubleColumn(valueColumn));
  }
...
//  }
    ...
    for (int i = 0; i < rowSubset.length; i++) {
      ...
      for (int col = 0; col < columns.length; col++) {
        switch (columnTypes[col]) {
          ...
          case FLOAT: newbie.setDouble(i, col, getDouble(row, col)); break;
  ...
  /**
   * Searches the entire table for double values.
   * Returns missing double (Double.NaN by default) if no valid numbers found.
   */
  protected double getMaxDouble() {
    boolean found = false;
    double max = PConstants.MIN_FLOAT;
    for (int row = 0; row < getRowCount(); row++) {
      for (int col = 0; col < getColumnCount(); col++) {
        double value = getDouble(row, col);
        if (!Double.isNaN(value)) {  // TODO no, this should be comparing to the missing value
          if (!found) {
            max = value;
            found = true;
          } else if (value > max) {
    ...
    return found ? max : missingDouble;
  }
  ...
  protected void convertRow(DataOutputStream output, String[] pieces) throws IOException {
    ...
    for (int col = 0; col < pieces.length; col++) {
      ...
      case LONG:
        ...
        break;
      case FLOAT:
        output.writeDouble(PApplet.parseDouble(pieces[col], missingDouble));
        break;
      case DOUBLE:
        try {
