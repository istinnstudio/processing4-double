...
/**
 * A simple class to use a <b>String</b> as a lookup for a double value. String "keys"
 * are associated with doubleing-point values.
 *
 * @webref data:composite
 * @webBrief A simple table class to use a <b>String</b> as a lookup for a double
 *           value
 * @see IntDict
 * @see StringDict
 */
public class FloatDict {
  ...
  public FloatDict(BufferedReader reader) {
    ...
    for (int i = 0; i < lines.length; i++) {
      ...
      if (pieces.length == 2) {
        keys[count] = pieces[0];
        values[count] = PApplet.parseDouble(pieces[1]);
   ...
   * @nowebref
   */
  public FloatDict(String[] keys, double[] values) {
   ...
   * each of the values into it. The version with the <b>double[]</b> parameters
  ...
  public double sum() {
    ...
    if (amount > Double.MAX_VALUE) {
      throw new RuntimeException("sum() exceeds " + Double.MAX_VALUE + ", use sumDouble()");
    }
    if (amount < -Double.MAX_VALUE) {
      throw new RuntimeException("sum() lower than " + -Double.MAX_VALUE + ", use sumDouble()");
    }
...
//    indices.put(keys[b], Integer.valueOf(b));
   ...
   * @return a DoubleDict with the original keys, mapped to their pct of the total
   */
  public DoubleDict getPercent() {
    ...
    for (int i = 0; i < size(); i++) {
      ...
      outgoing.set(key(i), (double) percent);
    }
    return outgoing;
  }
