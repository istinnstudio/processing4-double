...
public class PVector implements Serializable {
   ...
   * variables, the data from a <b>PVector</b>, or the values from a double array.
   *
   *
  ...
  /**
   *
   * Multiplies a vector by a scalar. The version of the method that uses a double
   * acts directly on the vector upon which it is called (as in the first example
   * above). The versions that receive both a <b>PVector</b> and a double as arguments are
  ...
  /**
   *
   * Divides a vector by a scalar. The version of the method that uses a double
   * acts directly on the vector upon which it is called (as in the first example
   * above). The version that receives both a <b>PVector</b> and a <b>double</b> as arguments is
  ...
  public PVector setHeading(double angle) {
    double m = mag();
    x = (double) (m * Math.cos(angle));
    y = (double) (m * Math.sin(angle));
    return this;
  }
   ...
   * while the latter takes three double values. (See the top and bottom examples
   * above, respectively.)
   *
   *
   * @webref pvector:method
   ...
   * @webref pvector:method
   * @usage: web_application
   * @webBrief Return a representation of the vector as a double array
   */
  public double[] array() {
    if (array == null) {
      array = new double[3];
